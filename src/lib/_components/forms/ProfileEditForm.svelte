<script lang="ts">
	...
	import { appstate, userbucketstate } from "$lib/_stores/auth_store";
    ...
    const fphone = field('phone', $appstate.account?.prefs.phoneNumber, [pattern(phoneRegEx)]);
    const fdob = field('dob', $appstate.account?.prefs.dob, []);
    const fgender = field('gender', $appstate.account?.prefs.gender);
    const faddress = field('address', $appstate.account?.prefs.address);
    const fdistrict = field('district', $appstate.account?.prefs.district);
    const ffile = field('uploader', $appstate.account?.prefs.photoID);
    const editProfileForm = form(fphone,fdob,fgender,faddress,fdistrict,ffile);

    // Status of form, Loaders
    let _saving = $state<boolean>(false);
    ...
    async function submitEditedForm(){
        ...
        try {
            ...
            await appstate.updateUserPrefs(prefs);
            modalStore.clear();
...
<main class=" w-full">
    <div class=" bg-surface-100 dark:bg-slate-800 p-6 rounded-2xl mx-auto w-full sm:w-3/4 lg:w-[48rem]">
        <h3 class="title">Edit { getFirstName($appstate.account?.name ?? 'User')}'s Details</h3>

        <form onsubmit={submitEditedForm}>
            <div class="my-6 grid grid-cols-4 gap-3">
                <div onclick={()=>document.getElementById('uploader')?.click()} onkeypress={null} aria-label="div uploader" class="col-span-4 flex flex-col gap-2 items-center justify-center bg-surface-300 rounded-xl p-6 dark:text-surface-700">
                ...
                <div class="col-span-4 md:col-span-2">
                    ...
                    <input type="tel" id="phoneNumber" placeholder="260" inputmode="tel" oninput={validateNumericInput} bind:value={$fphone.value}>
                </div>
            ...
            <section class="flex justify-end gap-3">
                <button type="button" class="btn btn-lg" onclick={()=>modalStore.close()}>Close</button>
