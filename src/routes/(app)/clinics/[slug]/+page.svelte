<script lang="ts">
	...
	import { clinicstate, petbucketstate, appstate } from '$lib/_stores/auth_store.js';
    ...
    let services;
    let clinic = $state<IClinic>();
    let clinicPhotos = $state<string[]>([]);
    let clinicServices = $state<{title: string, description: string}[]>([]);

    const defaultImage = '/images/vet.jpg';

    // Loaders
    let _loading: boolean = $state(false);
    let _loadingPhotos: boolean = $state(false);
    ...
    function verifyPermission(): boolean {
        if(!clinic || !$appstate.account) return false;
        console.log('Current User ID: ',$appstate.account?.$id);
        console.log('Clinic Owner ID: ',clinic.ownerID);
        
        return clinic.ownerID === $appstate.account?.$id ? true: false;
    }

    $effect(() => {
        // This will re-run whenever `clinic` changes
    ...
    <div class="w-full h-[14rem] rounded-2xl overflow-hidden mb-6">
        ...
        {#if _loadingPhotos || clinicPhotos.length < 1}
            ...
            <div class="h-full w-full relative" out:fade={{ duration:100 }}>
                ...
                {#if verifyPermission()}
                <button onclick={()=>{console.log('Modal to upload images')}} aria-label="Prompt to upload photos" class="carousel-text cursor-pointer">
                    <h2 class="m-0 font-medium">Upload photos of your Clinic</h2>
                    <p class="m-0 text-sm">click here to add photos</p>
                </button>
                {/if}
            </div>
        {:else if $appstate.account?.emailVerification}
