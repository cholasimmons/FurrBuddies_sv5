<script lang="ts">
	...
	import { petbucketstate, petstate, appstate } from '$lib/_stores/auth_store.js';
	import { onMount } from 'svelte';
	import { fade } from 'svelte/transition';
    import { Table, tableMapperValues, type ModalSettings, type ModalComponent, Avatar, type PopupSettings, popup, getModalStore } from '@skeletonlabs/skeleton';
    ...
    onMount(async ()=>{
        ...
        if(appstate.checkVerificationStatus() === false) goto('/pets', {replaceState:true});
        ...
        if(bud.photoID!.length > 0){
            _finding = true;
            try {
                const file: string|undefined = await petbucketstate.getPreview(bud.photoID![0]);
                imageURL = file ?? '';
                hasPhoto = true;
                _finding = false;
            } catch (error) {
        ...
        QRcodeURL = sdk.avatars.getQR(bud.$id, 128);
...
<main>
    <div class="w-full flex justify-evenly items-center py-6 relative ">
        ...
        <button onclick={viewPhoto} class="w-48 absolute top-2">
