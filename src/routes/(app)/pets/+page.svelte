<script lang="ts">
    ...
    import { appstate, petstate, petbucketstate } from '$lib/_stores/auth_store.js';
    import 'iconify-icon';
...
<main class="px-{data.padding}">
    ...
    <h3 class="title flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
            {#if $appstate.account}
                <button in:fade={{ duration:100, delay:100}} onclick={()=>goto('/pets/add')}  type="button" class="btn btn-sm variant-filled-secondary">
                    <span class=" flex items-center"><iconify-icon icon="mdi:paw"></iconify-icon></span>
                    <span>Add Buddy</span>
                </button>
            {:else}
                <button in:fade={{ duration:100, delay:100}} onclick={()=>goto('/auth/login')}  type="button" class="btn btn-sm variant-filled-surface">
            ...
            {#if !$petstate.pets}
                ...
                {:else if !$appstate.account?.emailVerification}
                    <span in:fade={{ duration: 300, delay: 300 }} out:fade={{ duration:200 }} class="text-sm flex flex-col">Your account is not verified
                        <button class="text-500 btn gap-2" onclick={resendActivationEmail} disabled={_sent}>
